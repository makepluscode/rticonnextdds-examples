...
int process_data(dds::sub::DataReader<DiscoverySnapshot> reader)
{
    // Take all samples
    int count = 0;
        ...
        unsigned int sample_count)
{
    ...
    dds::sub::DataReader<DiscoverySnapshot> reader(subscriber, topic);
    ...
    // Take discovery snapshots and save to files
    // Wait a moment to ensure discovery has completed
    rti::util::sleep(dds::core::Duration(2));
    
    const std::string participant_snapshot = "subscriber_participant_snapshot.xml";
    const std::string reader_snapshot = "subscriber_datareader_snapshot.xml";
    
    std::cout << "\n=== Taking Discovery Snapshots ===" << std::endl;
    std::cout << "Participant snapshot: " << participant_snapshot << std::endl;
    rti::util::discovery::take_snapshot(participant, participant_snapshot);
    
    std::cout << "DataReader snapshot: " << reader_snapshot << std::endl;
    rti::util::discovery::take_snapshot(reader, reader_snapshot);
    std::cout << "=== Snapshots saved ===\n" << std::endl;
}
